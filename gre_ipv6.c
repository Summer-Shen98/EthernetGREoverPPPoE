...
static int lock_ip(const char *ip) {
    ...
    char path[512];
    snprintf(path, sizeof(path), "%s/%s.lock", lock_dir, safe);
    ...
    if (len >= (int)sizeof(struct ip6_hdr)) {
        ...
        if (ip6->ip6_nxt == GRE_PROTOCOL) {
            ...
            if (inet_ntop(AF_INET6, &ip6->ip6_src, src, sizeof(src)) == NULL) {
                loge("inet_ntop failed: %s", strerror(errno));
            } else {
...
static void usage(const char *prog) {
    fprintf(stderr,
        ...
        "  IPv6 version. default queue_id=0, timeout=50s, script=./eogre_v6_pppoe.sh\n", prog);
}
...
int main(int argc, char **argv) {
    ...
    if (nfq_bind_pf(h, AF_INET6) < 0) {
        loge("nfq_bind_pf(AF_INET6) failed: %s", strerror(errno));
        nfq_close(h);
        return 2;
    }
